...
subir_s3 <- function(id_shipping,response_envio,tipo,carpeta){
    ...
    for(aux in productos){
      ...
      if(length(aux$fields$qr_image)==0 || nueva){
        if(!is.null(aux)){
          ...
          print(url_etiqueta)
          airtable_updatesinglerecord(list('qr_image' = list(list('url'= url_etiqueta))),
                                      'productos',Sys.getenv("AIRTABLE_CES_BASE"),aux$id)
          if(!last_response()$status_code %in% c(199:299)){
            print("Hubo un error:")
            print(last_response() %>% resp_body_json())
            break
        ...
        url_etiqueta <- aux$fields$qr_image[[1]]$url
      }
      ezeep_printbyurl(url_etiqueta,ezeep_at,"impresora_rosa",,copies = cantidad)
    }
  }
